...
# Copy Windows jextract output when present (skipped for ARM64-only DLL builds)
if (Test-Path C:\build\java-src) {
    $javaSrcFiles = Get-ChildItem C:\build\java-src -ErrorAction SilentlyContinue
    if ($javaSrcFiles) {
